...
// to the chart. We classify them via the CWE → ATT&CK lookup in
// `services/mitre/cweToAttack.ts`.
//
// Resolution order (first match wins, encoded in classifyVulnerability):
//   1. Direct CWE input (cweIds[]) — most authoritative
//   2. CWE extraction from CVE description text — common in NVD / Wazuh
//      VD prose ("[CWE-269]", "(CWE-89: SQL Injection)")
//   3. Keyword + CVSS heuristic — long-tail safety net for descriptions
//      with no CWE callout
//
// Wazuh VD docs lack a structured CWE field, so we lean on path 2 +
// path 3. Every classification carries `synthesized: true` so the UI
// can mark these as "(inferred from CVE)" — operators can tell whether
// MITRE on a row came from Wazuh detection (event evidence) or our
// classifier (state inference). Event-only predicates
// (runtime-attack / runtime-exploit / activeAttacks24h) skip
// scanner=wazuh-vd findings so synthesized MITRE never contaminates
// attack counters.
//
// Honest "no classification" (returns null) beats false-precise labels
// when neither the CWE lookup nor the heuristic matches.
function vulnToMitre(v: WazuhVulnerabilityDoc["vulnerability"]) {
  return classifyVulnerability({
    cveId:       v.id,
    description: v.description ?? "",
    cvssScore:   typeof v.score?.base === "number" ? v.score.base : 0,
  });
}
